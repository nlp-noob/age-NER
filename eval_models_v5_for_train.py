...
class Evaluator:
    ...
    def eval_bottom_line_test(self):
        ...
        print("TF = "+str(True_P)+"\t\t"+"FP = "+str(False_P)+"\t\t"+"FN = "+str(False_N))
        Precision, Recall, F1 = self._get_metrics(True_P, False_P, False_N)
        print("The I-DATE Precision is:{}".format(Precision), end="\t")
    ...
    def eval_bottom_line(self):
        ...
        for item in self.inputs:
            ...
            write_bad_case_flag = False
            for true_label, pred_label in zip(per_y_true, per_y_pred):
                if not pred_label in ["O", "I-DATE", "B-DATE", "DATE"]:
                    if(true_label!="O"):
                        False_N += 1
                        total_tags += 1
                    continue
    ...
    def write_badcase(self):
        ...
        with open("badcases/{}_Win{}_{}.txt".format("mymodel", win_size, input_type),  "w") as bf:
...
def main():
    ...
    with open("config.yaml","r") as stream:
        ...
        config["DATA_FILE_PATH"] = "data/small_json/valid.json"
